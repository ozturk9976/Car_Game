...
public class CineExtension : MonoBehaviour
{
    ...
    public void GameOverCamera(Transform collidedObject, Transform newFollowObj)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            var transposer = vcam.GetRig(i).GetCinemachineComponent<CinemachineOrbitalTransposer>();
            var composer = vcam.GetRig(i).GetCinemachineComponent<CinemachineComposer>();
